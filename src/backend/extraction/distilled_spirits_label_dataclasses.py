        ...
        def visit_BinOp(self, node):
            if isinstance(node.op, ast.BitOr):
                def collect(n):
                    ...
                    return [n]
                elts = [self.visit(e) for e in collect(node)]
                return ast.Subscript(
                    value=ast.Name(id='Union', ctx=ast.Load()),
                    slice=ast.Tuple(elts=elts, ctx=ast.Load()) if len(elts) > 1 else elts[0],
                    ctx=ast.Load()
                )
            return self.generic_visit(node)
            
    _transformer = UnionTransformer()
    
    def _transform_str(s):
