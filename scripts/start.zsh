...
if [ -n "$API_SERVICE" ]; then
  aws ecs update-service --cluster $CLUSTER_NAME --service $API_SERVICE --desired-count 1 > /dev/null
  echo "Started API service"
fi

if [ -n "$WORKER_SERVICE" ]; then
  aws ecs update-service --cluster $CLUSTER_NAME --service $WORKER_SERVICE --desired-count 1 > /dev/null
  echo "Started Worker service"
fi
